<template>
		<div class="topDiv" align="center">
			<div @dragover="dragOver(0)" >
				...
				<img  :src="getSrc(val,index,cardList[0])" :dd="index==0" :draggable="index==0||index==cardList[0].length-1" @dragstart="currentTop(index,0)" 
		...
		<div  width="5%" @dragenter="currentTop(cardList[1].length-1,1)"  >
		 ...
		 <div >游戏说明：拖动两列边缘的纸牌到其他边缘放置的位置，直到卡牌形成两列同花色从小到大的两行，游戏结束</div>
		 <div >共计次数：{{mtime}}<br/><img  src="./card.img/win.png" :hidden="!win"/></div>
		</div>
...
export default {
	...
	methods:{
		...
		checkWin(){
			this.win = (compareFull(this.targetOrder[0],this.cardList[1])&&compareFull(this.targetOrder[1],this.cardList[0])) ||
			(compareFull(this.targetOrder[1],this.cardList[1])&&compareFull(this.targetOrder[0],this.cardList[0]))
		},
